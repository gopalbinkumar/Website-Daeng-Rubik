                    ...
                    <tbody>
                        @foreach ($rows as $row)
                            ...
                            <tr class="result-row" data-category="{{ $categoryName }}"
                                ...
                                data-rank="{{ $row->rank ?? '-' }}" data-name="{{ ucwords(strtolower($participantName)) }}"
                                data-a1="{{ $row->attempt1 ?? 'DNF' }}" data-a2="{{ $row->attempt2 ?? 'DNF' }}"
                                data-a3="{{ $row->attempt3 ?? 'DNF' }}" data-a4="{{ $row->attempt4 ?? 'DNF' }}"
                                data-a5="{{ $row->attempt5 ?? 'DNF' }}" data-average="{{ $row->average ?? '-' }}"
                                data-best="{{ $row->best ?? '-' }}">
                                ...
                                <td class="text-start name-cell">
                                    {{ ucwords(strtolower($participantName)) }}
                                </td>
...
@once
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const modalBackdrop = document.getElementById('attemptModalBackdrop');
            if (modalBackdrop) {
                modalBackdrop.addEventListener('click', closeAttemptModal);
            }
        });
