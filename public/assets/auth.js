  ...
  if (confirmPassword && passwordInput) {
    confirmPassword.addEventListener('input', () => {
      if (confirmPassword.value && confirmPassword.value !== passwordInput.value) {
        ...
        const errorMsg = confirmPassword.parentElement.querySelector('.form-error');
        if (errorMsg) {
          errorMsg.style.display = 'flex';
        }
      } else if (confirmPassword.value === passwordInput.value && confirmPassword.value.length > 0) {
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', (e) => {
      ...
      if (submitBtn) {
        ...
        setTimeout(() => {
          alert('Akun berhasil dibuat! (UI Demo - belum ada backend)');
